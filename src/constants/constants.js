...
const experiences = [
  {
    ...
    points: [
      "Architected the Verve Identity Graph (VIG) — a BigQuery-backed identity resolution pipeline consolidating device IDs, HEM, and Experian signals from Sessionizer and partner sources, with automated GCS export feeding downstream audience activation and ad serving.",
      "Engineered Spark-based audience amplification to fan all DSP segments against the unified VIG graph, enabling cross-device targeting at scale and replacing fragmented per-vendor silo graphs across every onboarded partner.",
      "Integrated Gemini LLM into the taxonomy classification service and prototyped RAG-based enrichment for the Fusion Persona AI API; built MLOps infrastructure for the ATOM SDK — phased rollouts, automated Android config validation, and GCS sync on approval; designed BigQuery upsert strategy for ATOM feature vectors, eliminating flat-file proliferation.",
      "Onboarded programmatic DSP partners end-to-end — daily S3 ingestion, Kubernetes cron via ArgoCD, Taxonomy API sync to MySQL, and offline audience exports for real-time bidding (RTB) on the P+ stack.",
      "Designed and built the GenAI-powered suggestions API for Fusion Labs (audience exploration product) — accepts raw insights context and fusionId, calls an internal LLM service, and returns dynamic audience recommendations to partners and internal teams.",
      ...
      "Contributed to an Agentic AI monitoring tool — a configuration-driven agent that reasons over end-to-end production system state, auto-generates contextual alerts, and surfaces debugging guidance, reducing manual investigation overhead.",
    ],
  },
  {
    title: "Software Engineer",
